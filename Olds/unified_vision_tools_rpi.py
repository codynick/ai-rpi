...
def run_emotion(image_path: str):
    """
    FER+ ONNX with face alignment (InsightFace 5-point), tight crop, and simple [0,1] scaling.
    This reduces 'always neutral' by giving the model a canonical, high-contrast face.
    """
    ...
    try:
        ...
        if faces:
            ...
            # small margin to keep context
            cx = (x1 + x2) * 0.5; cy = (y1 + y2) * 0.5
            s = 1.05 * max(x2 - x1, y2 - y1)
            x1 = int(max(0, cx - s / 2)); y1 = int(max(0, cy - s / 2))
            x2 = int(min(W - 1, cx + s / 2)); y2 = int(min(H - 1, cy + s / 2))
            used_box = (x1, y1, x2, y2)

            # 5-point landmarks (left-eye, right-eye, nose, left-mouth, right-mouth)
            # Some insightface builds expose f.kps (5x2). If not present, skip alignment.
            kps = getattr(f, "kps", None)
            if kps is not None and np.shape(kps) == (5, 2):
                src = kps.astype(np.float32)

                # reference 5pts for 112x112 alignment (ArcFace)
                ref = np.array(
                    [[38.2946, 51.6963],
                     [73.5318, 51.5014],
                     [56.0252, 71.7366],
                ...
                if M is not None:
                    aligned112 = cv2.warpAffine(img0, M, (112, 112), flags=cv2.INTER_LINEAR)
                    # center-crop to 96x96 then down to 64x64 to keep central face
                    c0 = 8  # 112 -> 96 crop margin
                    crop96 = aligned112[c0:112-c0, c0:112-c0]
                    roi = cv2.resize(crop96, (64, 64), interpolation=cv2.INTER_AREA)
            # if alignment failed, use the tight box
            if roi is None:
    ...
    # --- grayscale + simple [0,1] scaling (no standardization) ---
